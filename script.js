...
pull_button.style.display = "none";
table.style.backgroundImage = "url('img/classic/table.jpg')";

window.onload = function () {
  const lastWinner = localStorage.getItem("winner");
  const message = document.getElementById("status_message");
  if (lastWinner) {
    message.textContent = "Last winner: " + lastWinner;
  } else {
    message.textContent = "No previous winner yet!";
  }
};
...
function animateCardMove(card, targetEl) {
  return new Promise((res) => {
    ...
    requestAnimationFrame(() => {
      ghost.style.transform = `translate(${end.left - start.left}px,${
        end.top - start.top
      }px)`;
    });
...
async function get_result(selected_card) {
  shuffle_button.style.display = "none";
  pull_button.style.display = "block";
...
function gameover() {
  if (player_score > ai_score) {
    ...
    localStorage.setItem("winner", "ai");
